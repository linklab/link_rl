    ...
    def make_gym_env(env_name):
        def _make():
            ...
            #############
                ...
                env = gym.make(env_name, frameskip=4, repeat_action_probability=0.0)
...
def get_single_env(config, no_graphics=True, play=False):
    ...
    #############
        ...
        if play:
            single_env = gym.make(config.ENV_NAME, render_mode="human", frameskip=4, repeat_action_probability=0.0)
        else:
            single_env = gym.make(config.ENV_NAME, frameskip=4, repeat_action_probability=0.0)
