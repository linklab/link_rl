...
class Buffer:
    ...
    def append(self, transition):
        if self.config.BUFFER_CAPACITY > 0:
            ...
            self.next_observations_buffer[self.head] = torch.from_numpy(transition.next_observation).to(self.config.DEVICE)   # TENSOR
            ...
            self.observations_buffer = torch.cat(# TENSOR
                (self.observations_buffer, torch.unsqueeze(torch.from_numpy(transition.observation).to(self.config.DEVICE), dim=0)), dim=0
            )

            if isinstance(self.action_space, Discrete):
                self.actions_buffer = torch.cat(
                    (self.actions_buffer, torch.full((1,), fill_value=transition.action, device=self.config.DEVICE)), dim=0
                )
            elif isinstance(self.action_space, Box):
                self.actions_buffer = torch.cat(
                    (self.actions_buffer, torch.unsqueeze(torch.from_numpy(transition.action).to(self.config.DEVICE), dim=0)), dim=0  # TENSOR
                )
            else:
                raise ValueError()

            self.next_observations_buffer = torch.cat(  # TENSOR
                (self.next_observations_buffer, torch.unsqueeze(torch.from_numpy(transition.next_observation).to(self.config.DEVICE), dim=0)), dim=0
            )
            ...
            self.dones_buffer = torch.cat(
                (self.dones_buffer, torch.full((1,), fill_value=bool(transition.done), device=self.config.DEVICE)), dim=0
            )
...
class HerEpisodeBuffer:
    ...
    def reset(self):
        self.episode_buffer = Buffer.get_new_buffer_without_capacity()
...
def cnn_buffer_test(config):
    observation_space = gym.spaces.Box(
        low=np.float32([[[-10, -10], [-10, -10]], [[-10, -10], [-10, -10]], [[-10, -10], [-10, -10]]]), # shape: 3, 2, 2
