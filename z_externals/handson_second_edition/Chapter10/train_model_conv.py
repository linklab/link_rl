...
if __name__ == "__main__":
    ...
    def sync_eval(engine: Engine):
        ...
        if engine.state.best_mean_val < mean_val:
            print("{0}: Best mean value updated {1:.3f} -> {2:.3f}".format(
    ...
    def validate(engine: Engine):
        ...
        for key, val in res.items():
            engine.state.metrics[key + "_test"] = val
    ...
    test_metrics = [m + "_test" for m in validation.METRICS]
    test_handler = tb_logger.OutputHandler(tag="test", metric_names=test_metrics)
    tb.attach(engine, log_handler=test_handler, event_name=event)
