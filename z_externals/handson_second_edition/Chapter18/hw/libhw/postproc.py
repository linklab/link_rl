...
class PostPitchRoll:
    ...
    def __init__(self, buffer, pad_yaw):
        ...
        assert isinstance(buffer.sensors[0],
                          z_externals.handson_second_edition.Chapter18.hw.libhw.hw_sensors.lis331dlh.Lis331DLH)
        self.buffer = buffer
    ...
    def __iter__(self):
        for b_list in self.buffer:
            for b in b_list:
                data = z_externals.handson_second_edition.Chapter18.hw.libhw.hw_sensors.lis331dlh.Lis331DLH.decode(b)
