...
def d4pg_test_net(net, env, count=10, device="cpu"):
    rewards = 0.0
    steps = 0
    ...
    with ptan.common.utils.RewardTracker(writer) as tracker:
        with ptan.common.utils.TBMeanTracker(writer, batch_size=10) as tb_tracker:
            while True:
                ...
                actor_loss_v = -crt_net.distribution_to_q(crt_distr_v)
                ...
                if frame_idx % TEST_ITERS == 0:
                    ts = time.time()
                    rewards, steps = d4pg_test_net(act_net, test_env, device=device)
