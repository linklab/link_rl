...
class LearnerComparison:
    def __init__(self, run, agents, wandb_obj=None, config_c=None, comparison_stat=None):
        ...
        self.next_test_training_step = self.config_c.TEST_INTERVAL_TRAINING_STEPS
        self.next_console_log = self.config_c.CONSOLE_LOG_INTERVAL_TRAINING_STEPS
        #######################################
    ...
    def train_comparison_loop(self):
        ...
        while True:
            ...
            if self.training_step >= self.config_c.MAX_TRAINING_STEPS:
                for agent_idx, _ in enumerate(self.agents):
