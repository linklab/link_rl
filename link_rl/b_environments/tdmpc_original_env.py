...
class FrameStackWrapper(dm_env.Environment):
	def __init__(self, env, num_frames, from_pixels=True):
		self._env = env
		...
		if self._from_pixels:
			...
			self._pixels_key = 'state'

		wrapped_obs_spec = env.observation_spec()
		assert self._pixels_key in wrapped_obs_spec

		pixels_shape = wrapped_obs_spec[self._pixels_key].shape
...
class TimeStepToGymWrapper(object):
	def __init__(self, env, domain, task, action_repeat, from_pixels):
		if from_pixels:
			modality = 'pixels'
		else:
			modality = 'state'
...
def make_env(config):
	...
	"""
	assert (config.DOMAIN_NAME, config.TASK_NAME) in suite.ALL_TASKS
	env = suite.load(config.DOMAIN_NAME,
					 config.TASK_NAME,
					 task_kwargs={'random': 1},
					 visualize_reward=False)
	env = ActionDTypeWrapper(env, np.float32)
	env = ActionRepeatWrapper(env, config.ACTION_REPEAT)
	env = action_scale.Wrapper(env, minimum=-1.0, maximum=+1.0)

	if config.FROM_PIXELS:
		if (config.DOMAIN_NAME, config.TASK_NAME) in suite.ALL_TASKS:
			camera_id = dict(quadruped=2).get(config.DOMAIN_NAME, 0)
			render_kwargs = dict(height=84, width=84, camera_id=camera_id)
			env = pixels.Wrapper(env,
								pixels_only=True,
								render_kwargs=render_kwargs)
		env = FrameStackWrapper(env, config.FRAME_STACK, config.FROM_PIXELS)
