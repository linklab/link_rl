...
class AgentGA(BaseAgent):
    def __init__(self, worker_id, input_shape, num_outputs, env, params, device):
        ...
        self.population = [
            (model, self.evaluate(model)) for model in self.models
        ]
    ...
    def next_generation(self):
        ...
        for _ in range(self.params.POPULATION_SIZE - 1):
            parent_model_idx = np.random.randint(0, self.params.PARENTS_COUNT)
            parent_model = prev_population[parent_model_idx][0]
            mutated_model = self.mutate_parent_model(parent_model)
            fitness = self.evaluate(mutated_model)
            self.population.append((mutated_model, fitness))
