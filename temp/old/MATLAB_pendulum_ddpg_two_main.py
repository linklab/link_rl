...
from common.environments import MatlabRotaryInvertedPendulumEnv, Status
...
from common.fast_rl import actions, policy_based_model, rl_agent, replay_buffer
...
##########################################################################################
    ...
    if params.PER:
        buffer_swing_up = replay_buffer.PrioritizedReplayBuffer(
            experience_source=None, buffer_size=params.REPLAY_BUFFER_SIZE, n_step=params.N_STEP
        )
        buffer_balancing = replay_buffer.PrioritizedReplayBuffer(
            experience_source=None, buffer_size=params.REPLAY_BUFFER_SIZE, n_step=params.N_STEP
        )
    else:
        buffer_swing_up = replay_buffer.ExperienceReplayBuffer(
            experience_source=None, buffer_size=params.REPLAY_BUFFER_SIZE
        )
        buffer_balancing = replay_buffer.ExperienceReplayBuffer(
            experience_source=None, buffer_size=params.REPLAY_BUFFER_SIZE
        )
